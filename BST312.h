...
template<class ItemType> BST_312<ItemType>::BST_312 ()
{
    root = NULL;
}

template<class ItemType> BST_312<ItemType>::BST_312(const BST_312 & src)
{
...
{
    //counts the total number of left and right nodes
    int totalNodes = 0;
    if(t == NULL){
        return 0;
    }
        ...
        totalNodes++;
        totalNodes+= countNodes(t->right);
        totalNodes+= countNodes(t->left);
        return totalNodes;
    }
}
...
{
    //searches tree nodes until item is found
